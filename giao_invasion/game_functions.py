...
def check_bullet_alien_collisions(ai_settings,screen,stats,sb,ship,aliens,bullets,giao):
    '''相应子弹和外星人碰撞'''
    #检查子弹是否击中敌人，若有，则删除相应的子弹和外星人
    collisions=pygame.sprite.groupcollide(bullets,aliens,True,True)  #两个true表示将碰撞的两个物体都删除掉
    if collisions:
        # giao 音效  （仅当alien图像用外星人代替时使用）
        giao.giao_wav.play()
...
def update_bullets(ai_settings,screen,stats,sb,ship,aliens,bullets,giao):
    '''更新子弹位置及清除消失的子弹'''
    bullets.update()
    #删除已消失的子弹
    ...
    check_bullet_alien_collisions(ai_settings, screen, stats, sb,ship, aliens, bullets,giao)
